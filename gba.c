...
void drawRectDMA(int row, int col, int width, int height, volatile u16 color) {
  // TODO: IMPLEMENT
  // why are we making it volatile? why can't we use &color?
	volatile u16 pcolor = color;
  for (int r = 0; r < height; r++) {
		DMA[3].src = &pcolor;
		// why is it screen width?
...
void undrawImageDMA(int row, int col, int width, int height, const u16 *image) {
	// BACKGROUND[ROW][COL]
	// first row of image
	// int initial_offset = OFFSET(row, col, WIDTH);
  	for (int r = 0; r < height; r++) {
  		// add one row
...
void fillScreenDMA(volatile u16 color) {
	volatile unsigned short pcolor = color;
	DMA[3].src = &pcolor;
	DMA[3].dst = videoBuffer;
